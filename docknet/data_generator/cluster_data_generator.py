...
class ClusterDataGenerator(DataGenerator):
    """
    The cluster data generator generates two classes (0 and 1) of 2D vectors
    ...
    @staticmethod
    def unitary_cluster(x: np.ndarray) -> np.ndarray:
        polar = random_to_polar(x)
        polar[0] = 11.**polar[0] / 10. - 0.6
        f = polar_to_cartesian(polar)
        return f

    def func0(self, x: np.ndarray) -> np.ndarray:
        """
        ...
        """
        f = (self.unitary_cluster(x) * self.cluster_diameter
             + self.cluster0_origin)
        return f

    def func1(self, x: np.ndarray) -> np.ndarray:
        """
    ...
    def __init__(self, x0_range: Tuple[float, float],
        ...
        self.cluster0_origin = np.array([x_center - x_length / 3,
                                         y_center + y_length / 3])
        self.cluster1_origin = np.array([x_center + x_length / 3,
                                         y_center - y_length / 3])
