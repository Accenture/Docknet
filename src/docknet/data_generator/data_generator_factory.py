...
class UnknownDataGeneratorName(Exception):
    def __init__(self, data_generator_name: str):
        message = f'Unknown data generator name {data_generator_name}'
        super().__init__(message)


def make_data_generator(data_generator_name: str,
                        x0_range: Tuple[float, float],
                        x1_range: Tuple[float, float]):
    """
    Given the name of a data generator and the ranges of the vectors they are
    to generate, create and return the corresponding data generator instance
    ...
    """
    try:
        generator_class = data_generators[data_generator_name](x0_range,
                                                               x1_range)
    except KeyError:
