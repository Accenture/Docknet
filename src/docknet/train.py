...
def parse_args():
    ...
    parser.add_argument('--model-in', '-mi', action='store', required=True,
                        help='Initial model file in JSON or pickle format '
                             'describing the Docknet layers and, optionally, '
                             'the initial parameters')
    parser.add_argument('--model-out', '-mo', action='store', required=True,
                        help='Model file (JSON or pickle) where to write the '
                             'resulting Docknet model after training')
    parser.add_argument('--initializer', '-i', action='store', default=None,
                        help='Initializer to use (do not specify, for no '
                             'initialization, or specify random)')
    parser.add_argument('--optimizer', '-o', action='store', default='adam',
                        help='Optimizer to use (defaults to adam)')
    parser.add_argument('--batch-size', '-b', action='store', required=True,
                        type=int,
                        help='Amount of training examples to use per training '
                             'iteration')
    parser.add_argument('--max-number-of-epochs', '-e', action='store',
                        required=True, type=int,
                        help='Maximum number of epochs to train')
    parser.add_argument('--error-delta', '-d', action='store', default=0.0,
                        type=float,
                        help='Error increment threshold (defaults to 0, '
                             'meaning there is no threshold)')
    parser.add_argument('--max-epochs-within-delta', '-ed', action='store',
                        default=-1, type=int,
                        help='Maximum number of epochs with error increment '
                             'within the specified threshold (defaults to -1, '
                             'meaning there is no maximum')
    parser.add_argument('--stop-file', '-s', action='store', default=None,
                        help='File whose existence will end the training '
                             'process (defaults to none); at any time during '
                             'training, create this file (e.g. with command '
                             'touch) so no more epochs are run and the model '
                             'is saved')
    ...
    if not os.path.exists(args.trainingset):
        print(f'Trainingset file {args.trainingset} doesn\'t exist')
        sys.exit(1)

    if not os.path.exists(args.model_in):
        print(f'Initial model file {args.model_in} doesn\'t exist')
        sys.exit(1)

    args.model_in_type = os.path.splitext(args.model_in.lower())[1]
    if args.model_in_type not in ['.json', '.pkl']:
        print('Initial model file must be either a JSON or a pickle file')
        sys.exit(1)

    args.model_out_type = os.path.splitext(args.model_out.lower())[1]
    if args.model_out_type not in ['.json', '.pkl']:
        print('Output model file must be either a JSON or a pickle file')
        sys.exit(1)
...
def main():
    ...
    if args.initializer:
        ...
        if args.initializer == 'random':
            ...
            print(f'Unknown initializer {args.initializer}; available '
                  f'initializers: random')
    if args.optimizer == 'adam':
        docknet.optimizer = AdamOptimizer()
    elif args.optimizer == 'gradient_descent':
        docknet.optimizer = GradientDescentOptimizer()
    else:
        print(f'Unknown optimizer {args.optimizer}; available optimizers: '
              f'adam, gradient_descent')
        sys.exit(1)
    docknet.cost_function = 'cross_entropy'
    docknet.train(X, Y, args.batch_size,
                  ...
                  stop_file_pathname=args.stop_file, initialize=initialize)
