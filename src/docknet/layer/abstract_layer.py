...
class AbstractLayer(ABC):
    def __init__(self, dimension: int, params: Dict[str, np.ndarray]):
        """
        ...
        """
        # Initialize '_p', the layer's dictionary of parameters, bypassing
        # method __setattr__; note that since the parameter dictionary doesn't
        # exist yet, __setattr__ would fail if not bypassed since it will try
        # to access the '_p' to check if '_p' is a layer parameter
    ...
    @params.setter
    def params(self, p: Dict[str, np.ndarray]):
        self._p = p

    def __getattr__(self, attr: str):
        """
        Let l be a layer object and p be a parameter name of l, this method
        allows for getting p's value with notation
            l.p
        apart from the more cumbersome notation
            l.params['p']
        """
        # This function is called if attr is not a member of the class, so we
        # simply try to get that attribute from the dictionary of parameters.
        # If neither found there, then throw the standard exception
        try:
    ...
    def __setattr__(self, attr: str, value: Any):
        """
        Let l be a layer object, p be a parameter name of l, and v the value to
        assign to p, this method allows for setting p's value with notation
            l.p = v
        apart from the more cumbersome notation
            l.params['p'] = v
        """
        # If attribute name is a key in the dictionary of parameters of this
        # layer, assign it to that parameter key
