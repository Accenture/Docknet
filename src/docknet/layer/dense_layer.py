...
class DenseLayer(AbstractLayer):
    def __init__(self, previous_layer_dimension, layer_dimension: int, activation_function_name: str):
        """
        Initializes the dense layer, given the previous layer dimension, this layer dimension and the activation
        function to use
        :param previous_layer_dimension: the previous layer dimension (number of outputs)
        :param layer_dimension: this layer dimension (number of outputs)
        :param activation_function_name: the name of the activation function to use (e.g. "sigmoid", "relu", etc.)
        """
        params = {
    ...
    def forward_linear(self, A_previous: np.array) -> np.array:
        """
        Computes the linear part of this layer's forward propagation Z = W * A_previous + b
        :param A_previous: activation of the previous layer, or X (the input data) if this is layer 1
    ...
    def forward_propagate(self, A_previous: np.array) -> np.array:
        """
    ...
    def cached_forward_propagate(self, A_previous: np.array) -> np.array:
        """
        Performs the same computation than forward_propagate but also caches the values required to later
        perform the backward propagation, A_previous and Z
        :param A_previous: the output of the previous layer
        :return: A, the output of this layer
        """
    ...
    def backward_linear(self, dJdZ: np.array) -> Tuple[np.array, np.array, np.array]:
        """
        Given the partial derivative of the cost function J wrt Z, ∂J/∂Z, computes the following partial derivatives:
        1) ∂J/∂A_prev,
        2) ∂J/∂W, and
        3) ∂J/∂b
        :param dJdZ: ∂J/∂Z or partial derivative of the cost function J wrt Z, the linear part of this layer computed
        during the previous forward propagation
    ...
    def backward_activation(self, dJdA: np.array) -> np.array:
        """
        Given the partial derivative of the cost function J wrt A, ∂J/∂A, and the linear part of this layer
        computed and cached during the previous forward propagation, Z, computes the following partial derivatives:
        1) ∂J/∂A_prev,
        2) ∂J/∂W, and
        3) ∂J/∂b
        :param dJdA: ∂J/∂A or partial derivative of the cost function J wrt A, the activation values of this layer
        during the previous forward propagation
        :return: ∂J/∂Z, the partial derivative of the cost function wrt Z, the linear part of this layer during the
    ...
    def backward_propagate(self, dJdA: Union[float, np.array]) -> Tuple[np.array, Dict[str, np.array]]:
        ...
        2) the gradients of J wrt each parameter of this layer, which are to be cached in the layer object
        :param dJdA: the gradient of the cost function wrt the activation of this layer
        :return: ∂J/∂A_prev and a dictionary with the gradients of J wrt the layer parameters W and b
        """
    ...
    def to_dict(self) -> Dict:
        """
        Converts this layer to a Python dictionary for JSON serialization (keeping the info needed to later create the
        layer back from the same Python dictionary)
        :return: a Python dictionary with all the info needed to create the layer back
        """
        d = super().to_dict()
